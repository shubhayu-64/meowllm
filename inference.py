"""MeowLM inference — talk to the cat."""
...
class MeowInference:
    def __init__(self, checkpoint_path, tokenizer_path, device="cpu"):
        ...
        print(f"MeowLM loaded: {total/1e6:.1f}M params")
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Chat with Meow")
    p.add_argument("--checkpoint", default="checkpoints/best_model.pt")
    p.add_argument("--tokenizer", default="data/tokenizer.json")
    p.add_argument("--device", default="cpu")
    args = p.parse_args()

    engine = MeowInference(args.checkpoint, args.tokenizer, args.device)
    print("\nMeow Chat (type 'quit' to exit)")
    print("(Warning: the cat might ignore you)")
    msgs = []
    while True:
        inp = input("\nYou> ").strip()
